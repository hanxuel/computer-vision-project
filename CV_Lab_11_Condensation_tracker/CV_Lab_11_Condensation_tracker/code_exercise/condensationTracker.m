...
for i = 1:length(frameValues)
    ...
    hist_current = color_histogram(min(max(1,meanStateAPosteriori(i,1)-0.5*WidthBB),widthFrame),min(max(1,meanStateAPosteriori(i,2)-0.5*HeightBB),heightFrame),min(max(1,meanStateAPosteriori(i,1)+0.5*WidthBB),widthFrame),min(max(1,meanStateAPosteriori(i,2)+0.5*HeightBB),heightFrame),frame,params.hist_bin);
