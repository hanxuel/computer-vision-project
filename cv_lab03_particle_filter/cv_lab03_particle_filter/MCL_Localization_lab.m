...
%
    ...
    %
    for i=1:size(priorbelief_particles,2)
        priorbelief_particles(i)=mod(priorbelief_particles(i)+uk+randn(1)*simpar.odometry_stdev,simpar.domain);
    end
...
%
    ...
    % normalization
    particle_weights = particle_weights / sum(particle_weights);  % normalization
end
...
%
    ...
    % change this for the correct method
    ...
    for i=1:simpar.numberOfParticles
        ...
        while(particle_weights(index)<beta)
            beta=beta-particle_weights(index);
            index=mod(index,simpar.numberOfParticles)+1;
        end
        updated_belief_particles=[updated_belief_particles,predict_particles(index)];
    end
end
